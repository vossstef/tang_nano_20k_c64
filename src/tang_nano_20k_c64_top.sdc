...
create_generated_clock -name mspi_clk -source [get_ports {clk}] -master_clock clk -divide_by 8 -multiply_by 19 -phase 135 [get_pins {flashclock/CLKOUTD}]
create_generated_clock -name flash_clk -source [get_ports {clk}] -master_clock clk -divide_by 8 -multiply_by 19 [get_pins {flashclock/CLKOUT}]
...
set_clock_groups -asynchronous -group [get_clocks {flash_clk}] -group [get_clocks {clk64}] -group [get_clocks {clk32}] -group [get_clocks {clk_pixel_x5}]
